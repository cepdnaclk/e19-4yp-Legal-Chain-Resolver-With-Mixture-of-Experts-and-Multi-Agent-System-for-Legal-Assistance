...
from src.moe.gating import TopKGatingNetwork

def final_query(query_text, top_k=3):
    """Queries the MoE system and returns the top results."""
    # --- Load Models and Mappings ---
    print("Loading models and mappings...")
    # Load subdomain to index mapping
    with open("data/subdomain_map.json", 'r') as f:
    ...
    gating_network.load_state_dict(torch.load("data/gating_model.pt"))
    gating_network.eval()
    ...
    for name, i in subdomain_map.items():
        ...
        expert_path = os.path.join("data/subdomains", name, "expert_model.pt")
    ...
    for i in range(top_k_indices.size(1)):
        ...
        subdomain_folder = os.path.join("data/subdomains", expert_name)
