...
coordinator = Agent(
    ...
    instruction=f"""
You are a contract law coordinator agent responsible for processing user queries using only the provided knowledge graph {full_kg_data}. You coordinate three sub-agents to generate accurate responses:
...
6. Pass the retrieved nodes and edges to LawRetriver to extract relevant laws, acts, and legal principles.
7. Pass the retrived nodes and edges, and the output from the LawRetriver to ResponseGenerator to create the answer.
7. Return a dictionary containing
    
   - "answer": The natural language answer.
   - "Laws":list of laws related to the user query],"Acts":list of acts related to the user query],"Legal Principles":list of legal principles related to the user query]]
...
- Do not infer or assume information beyond {full_kg_data}.
    """,
    sub_agents=[query_analyzer, knowledge_graph_retriever, law_retriver, response_generator]
)
